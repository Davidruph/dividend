...
app.post("/register", (req, res) => {
  ...
  if (fullname == "") {
    res.send({ message: "Pls enter your fullname" });
  } else if (email == "") {
    res.send({ message: "Pls enter an email" });
  } else if (!email.match(validRegex)) {
    res.send({ message: "Pls enter a valid email" });
  } else if (password == "") {
    res.send({ message: "Pls enter a password" });
  } else if (confirm == "") {
    res.send({ message: "Pls enter confirm password" });
  } else if (confirm !== password) {
    res.send({ message: "Your two passwords don't match" });
  } else {
    dbconn.query(
...
app.post("/glogin", (req, res) => {
  ...
  if (email === "") {
    res.send({ message: "Pls select your google account again" });
  } else {
    dbconn.query(
      "SELECT * FROM users WHERE email = ?;",
      email,
      (err, result) => {
        ...
        if (result.length > 0) {
          ...
          bcrypt.hash(password, saltRounds, (err, hash) => {
            dbconn.query(
              "INSERT INTO users (fullname, email, password, role, login_type) VALUES (?,?,?,?,?)",
              [fullname, email, hash, role, type],
              (err, result) => {
                console.log(err);
                if (result !== "") {
                  req.session.user = result;
